normalization <- function(ratioCorrectedBias, bedTools.dir, genotype.file, vcf = TRUE, working.dir = NULL,
  ...
  if(vcf==TRUE) {
    ...
    ff <- paste(bedTools.dir, "intersect"," -a ", file.path (working.dir, "tumor.MAF.highcut.bed")," -b ", file.path (working.dir, "Ratio.bed"),
               " -wa -wb > ",  file.path (working.dir, "tumor.MAF.ratio.bed"))
    system(ff)
  } else {
    ff <- paste(bedTools.dir,"intersect", " -a ", genotype.file, " -b ", file.path (working.dir, "Ratio.bed"),
                 " -wa -wb > ",  file.path (working.dir, "tumor.MAF.ratio.bed"))
    system(ff)
  }
  ##########
