createCentromereBins <- function(cytoband = NULL, bin.size = 100000, write = FALSE, result.dir = NULL){
  ...
  for(i in c(1:TargetAnnotations$numchrom , "X", "Y")){
    ...
    } else if (i ==TargetAnnotations$numchrom) {
      # message ("Not creating virtual centromere for chr",i," to leave room for X/Y")
    } else {
  ...
  if(write == TRUE){
    write.table(allbins, file.path (result.dir, paste0("centromere.bin", format(binsize, scientific= FALSE), ".bed")), col.names = F, row.names = F, quote = F, sep ="\t")
  }
